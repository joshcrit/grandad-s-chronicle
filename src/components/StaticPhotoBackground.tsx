// Static photo background - same images as carousel but no animation
...
export function StaticPhotoBackground() {
    ...
    <div className="fixed inset-0 w-full h-full overflow-hidden pointer-events-none z-0">
      <div className="absolute inset-0 flex flex-col">
        {rows.map((rowPhotos, rowIndex) => (
          <div key={rowIndex} className="flex flex-1 h-full">
            {rowPhotos.map((photoPath, index) => (
              <div
                key={`${photoPath}-${index}`}
